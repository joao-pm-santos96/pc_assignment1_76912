...
class MyRob(CRobLinkAngs):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.measures.collision:
                ...
                # TODO remove
                for cena in cenas:
                    print(np.unravel_index(cena.argmax(), cena.shape))
    ...
    #             for sensor in range(NUM_IR_SENSORS):  
    #                 expected *= self.ground_truth[row][col][sensor]
    #                 measured *= measures[sensor]

    #             probabilities_row.append(self._normalDistribution(measured, expected, self.sensor_noise) * self.cells_probability[row][col])

    #         probabilities_map.append(probabilities_row)

    #     # normalize
    #     self.cells_probability = np.divide(probabilities_map, np.sum(probabilities_map))
...
if __name__ == '__main__':
    ...
    if mapc != None:
        ...
        # cell = (1,4)
        # (pos, rot) = rob._getCellSensorsPoses(cell[0], cell[1])
        ...
        # for i in range(20):
        #     (pos, rot) = rob._getCellSensorsPoses(cell[0], cell[1]+i)
        #     # rob._getWallsCorners()

        #     print([round(x, 1) for x in rob._computeCellMeasures(cell[0], cell[1]+i)])

        # for i in range(NUM_IR_SENSORS):
        #     rob.plotMapAndRobot(pos[i], rot[i], rob.walls)
